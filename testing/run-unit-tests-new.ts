...
import {
  runAllUnitTests,
  UnitTestHarness,
} from "@wallet-test/rn-test-harness/node";

async function main() {
  console.log(
    "Starting React Native Unit Test Runner (using packaged harness)...",
  );
  ...
  try {
    ...
    console.log(
      `   Harness Summary: ${results.harnessResults.passed}/${results.harnessResults.total} passed`,
    );
...
async function runCustomTests() {
  ...
  try {
    ...
    await harness.runSingleTest(
      "globalThis.__TEST_MODULES__.testCount",
      "Test Count Check",
    );
