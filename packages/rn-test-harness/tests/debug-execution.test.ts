...
test('should connect to React Native debug target', async () => {
  ...
  expect(target.webSocketDebuggerUrl).toMatch(/^ws:\/\/localhost:8081\/inspector\/debug/);
});
...
test('should execute string operations', async () => {
  const result = await debugClient.evaluateJavaScript('"Hello" + " " + "World"');
...
test('should access React Native specific globals', async () => {
  ...
  const bridgeExists = await debugClient.evaluateJavaScript('typeof globalThis.nativeFabricUIManager !== "undefined" || typeof globalThis.__fbBatchedBridge !== "undefined"');
