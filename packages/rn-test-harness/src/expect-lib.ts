...
function createMatchers(actual: any, isNot: boolean = false): Matchers {
  ...
  Object.assign(matchers, {
    ...
    toStrictEqual: (expected: any) => {
      const pass = deepEqual(actual, expected);
      if (isNot ? pass : !pass) {
        throw new Error(`Expected ${JSON.stringify(actual)} ${isNot ? 'not ' : ''}to strictly equal ${JSON.stringify(expected)}`);
      }
    },
    ...
    toBeInstanceOf: (constructor: any) => {
      ...
      if (isNot ? pass : !pass) {
        throw new Error(`Expected ${actual} ${isNot ? 'not ' : ''}to be instance of ${constructor.name}`);
      }
    },
    ...
    toBeCloseTo: (expected: number, precision: number = 2) => {
      ...
      if (isNot ? pass : !pass) {
        throw new Error(`Expected ${actual} ${isNot ? 'not ' : ''}to be close to ${expected}`);
      }
    },
    
    toContain: (item: any) => {
      const pass = Array.isArray(actual) ? actual.includes(item) : actual.indexOf(item) !== -1;
      if (isNot ? pass : !pass) {
        throw new Error(`Expected ${actual} ${isNot ? 'not ' : ''}to contain ${item}`);
      }
    },
    
    toContainEqual: (item: any) => {
      const pass = Array.isArray(actual) ? actual.some(el => deepEqual(el, item)) : false;
      if (isNot ? pass : !pass) {
        throw new Error(`Expected ${JSON.stringify(actual)} ${isNot ? 'not ' : ''}to contain equal ${JSON.stringify(item)}`);
      }
    },
    ...
    toHaveLength: (expected: number) => {
      ...
      if (isNot ? pass : !pass) {
        throw new Error(`Expected ${actual} ${isNot ? 'not ' : ''}to have length ${expected}`);
      }
    },
    
    toHaveProperty: (keyPath: string, value?: any) => {
      const hasProperty = getProperty(actual, keyPath) !== undefined;
      const hasCorrectValue = value === undefined || deepEqual(getProperty(actual, keyPath), value);
      const pass = hasProperty && hasCorrectValue;
      if (isNot ? pass : !pass) {
        throw new Error(`Expected ${JSON.stringify(actual)} ${isNot ? 'not ' : ''}to have property ${keyPath}${value !== undefined ? ` with value ${JSON.stringify(value)}` : ''}`);
      }
    },
    ...
    toMatch: (expected: string | RegExp) => {
      ...
      if (isNot ? pass : !pass) {
        throw new Error(`Expected ${actual} ${isNot ? 'not ' : ''}to match ${expected}`);
      }
    },
    ...
    toThrow: (expected?: string | RegExp | Error) => {
      ...
      if (isNot ? pass : !pass) {
        if (!thrownError) {
          throw new Error(`Expected function ${isNot ? 'not ' : ''}to throw${expected ? ` ${expected}` : ''}`);
        }
