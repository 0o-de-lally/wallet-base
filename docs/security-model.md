...
| Password | 6-digit user password (will evolve to higher-entropy passphrase) | Sensitive |
...
This clarifies under what circumstances an attacker can obtain the encrypted mnemonic blobs and password hash from Expo SecureStore (Keychain / Keystore abstraction):
...
| Encrypted iOS backup acquisition | iOS | Access to encrypted iTunes/Finder backup file AND backup password (or iCloud account + MFA) | Keychain items (including stored ciphertext & password hash) | Treat possession of decrypted backup as privileged compromise; encourage encrypted backups |
...
| Android root / custom recovery / physical forensic imaging | Android | Rooted device / unlocked bootloader / physical forensic toolkit | EncryptedSharedPreferences file + keystore blobs (yielding ciphertext & password hash) | Hardware-backed keystore may protect raw keys; ciphertext still exposed |
| Live runtime instrumentation (Frida / debugger attach) | Both | Physical access + user device unlock & dev mode (or root/jailbreak) | Plaintext mnemonic at decryption moment; ciphertext & password hash trivially observable | Mitigate via native crypto, ephemeral buffers, debugger detection (optional) |
...
- Increase user secret entropy (stronger password / passphrase) to raise cost after privileged dump.
...
```
Mnemonic (plaintext, transient) --> Scrypt(password, randomSalt) --> 256-bit key
...
Password hash (Scrypt again w/ separate salt) stored as JSON (key to be obfuscated in future)
```

## 6. Current Defensive Controls
| Control | Purpose | Status |
|---------|---------|--------|
| Scrypt (N=32768,r=8,p=1) | Memory-hard KDF for password -> key derivation | Implemented |
...
|------|------------|--------------------|
| Low-entropy password | 1e6 search space | Increase length/allow passphrases + optional biometrics |
| No hardware key wrapping | Ciphertext usable if password brute forced | Introduce device master key & wrap DEK |
| Password hash & rate limit metadata predictable keys | Targeted tampering enumeration | Obfuscate / unify metadata storage |
| Plaintext mnemonic lifetime in memory | Interception window | Ephemeral buffer + immediate zeroization attempt |
| Overlay / accessibility phishing | Password capture | Biometric gating + UI detection + user education |
...
| Password policy upgrade + passphrase option | Exponential brute force cost increase | High |
