...
export function useTransactionPin({ accountId, onMnemonicRetrieved }: UseTransactionPinProps) {
  ...
  const handlePinSubmit = useCallback(async (pin: string) => {
    ...
    try {
      ...
      const decryptResult = await secureDecryptWithPin(encryptedMnemonic, pin);
      
      if (!decryptResult || !decryptResult.verified || !decryptResult.value) {
        showAlert("Error", "Failed to decrypt mnemonic. Invalid PIN or corrupted data.");
        setIsLoading(false);
        return;
      }
      ...
      reportErrorAuto("useTransactionPin.handlePinSubmit", error, { accountId });
      setIsLoading(false);
    }
