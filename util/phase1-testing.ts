  ...
  try {
    const tests = [
      ...
      { pin: "123456789012", shouldPass: true, name: "12-character PIN" },
      {
        pin: "1234567890123",
        shouldPass: false,
        name: "13-character PIN (too long)",
      },
      {
        pin: "123456!",
        shouldPass: false,
...
export async function testKeyObfuscation(): Promise<TestResult> {
  try {
    ...
    const isAccountKeyObfuscated =
      accountKey !== originalKey && accountKey.startsWith("obf_");

    const passed =
      isDeterministic && isObfuscated && hasPrefix && isAccountKeyObfuscated;
...
export async function testMigration(): Promise<TestResult> {
  try {
    ...
    const migrationRuns =
      typeof migration.success === "boolean" &&
  ...
  try {
    ...
    const tests = await Promise.all([
      ...
      checkLockoutStatus().then(
        (status) => typeof status.isLockedOut === "boolean",
      ),
